                                    ...
                                    int steps_on_one_eighth = 10)
{
    ...
    const FNUM_TYPE sin_power_epsilon__cutoffpoint =
        WITH_SUFFIX_fx(pow)(sin_cutoffpoint, epsilon);
    FSG_LOG_VAR(sin_power_epsilon__cutoffpoint);
         ...
         sin_power_epsilon += sin_power_epsilon__step)
    {
        FNUM_TYPE sin_squared =
            WITH_SUFFIX_fx(pow)(sin_power_epsilon, sg_2 / epsilon);

        // cos = sqrt(1 - sin^2)

        FNUM_TYPE cos_squared = (sg_1 - sin_squared);

        FNUM_TYPE cos_power_epsilon =
            WITH_SUFFIX_fx(pow)(cos_squared, epsilon / sg_2);
...
SuperEllipsoidParameters::toPointCloud(int steps)
{
    ...
    std::vector<std::complex<FNUM_TYPE>> superEllipseCoordsPitch =
        superEllipseParametersToPoint(1, dilatfactor_z, exp_1, -2, 2, steps);
...

    {
        ...
        for (const auto &exp_1 : stops)
        {
            for (const auto &exp_2 : stops)
            {
                ...
                //my_super_quadric.set_rot_yaw(sg_pi_two / 11);
                my_super_quadric.set_rot_pitch(sg_pi_two / 7);
                ...
                pcl::PointCloud<pcl::PointXYZ>::Ptr my_super_quadric_points =
                    my_super_quadric.toPointCloud(40);
            }
        }
